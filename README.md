...
- **Gボール**（左下・[ G ] ON 時） … スマホIMUから前後左右Gを表示。ボールは**体が押される方向**に動き（制動で上、右旋回で左）、**合成G**（ベクトル長）も併記。フルスケール ±0.5g。**校正が済むまではグレーアウト**で枠と案内だけ出る（最初から枠は見える）
...
| **[ G校正 ]** | スマホIMUによる**前後左右Gのボール表示**（FLOW図の左下）の姿勢校正。**停車中に押す**と重力方向を2秒学習し、続いて**次に前進で発進した方向を前方と認識**する（2段で姿勢が一意に決まる）。校正値は保存され次回起動時は自動で有効になる。スマホを付け替えたら押し直す。REC とは独立に使える。**ボタンのハイライトは校正シーケンス進行中のみ**で、完了後・[IMU] OFF 時・タイムアウト時（重力6秒／発進待ち60秒／発進計測10秒）は消える。**実行中にもう一度押すと中止**でき、中止・タイムアウト時は**前回の校正値に復帰**する。<br>**枠は最初からグレーアウトで表示され、中に次にやることが出る**（`[G校正]を押す` → `停車のまま…` → `発進してください`）ので、機能の存在と何が足りないかが分かる |
