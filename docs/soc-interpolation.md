...
`replay.html` の `parseFrames` で、5B到来時に SOC を再アンカーし、その間を 2920 の pbat 積算（K=−154 %/kWh）で前進させて `soc` を作る。グラフの SOC パネル（`socPts`）とフロー図の BAT ノード（`D.soc`）は `o.soc` を参照するため、内挿値が自動で反映される。ライブアプリ（`index.html`）にも同じ内挿を実装（表示専用、ログは生の 5B/2920 のみ）。
