...
| 1.3 | 2026-08 | 参照PIDの引用規格を SAE J1979-2 → **SAE J1979**（Classic OBD-on-ISO 15765-4）に訂正。本車はサービス01 PIDで応答＝J1979-2(OBDonUDS)には該当しないため。手法の汎用性の記述を「メーカー内部資料に依存せず」から改め、Honda 以外（トヨタ THS・日産 e-POWER 等）への応用可能性を明示（Abstract・§6・§8）。 |
