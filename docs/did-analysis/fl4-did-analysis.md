...
Honda はこれらDIDの構造を公開していない。そのため解析には、外部から観測できる情報のみを用いた。具体的には、標準OBD-II PID（SAE J1979）、車両の公表諸元（減速比・タイヤ径）、および実走行で取得した診断通信ログである。標準PIDとの相関、諸元から導いた運動学的拘束、エネルギー保存則に基づく物理的整合を組み合わせ、各信号を物理量へ対応付けた。
...
本手法は、公開情報と物理法則のみで未文書化の診断データを同定する。特定のメーカーや機構に依存しないため、Honda 以外の車両——トヨタ THS や日産 e-POWER などのハイブリッドを含む——や他ECUの未文書化DIDにも応用できる可能性がある。
...
- **標準PID（SAE J1979 準拠）**：車速・エンジン回転数・HV電池電圧/電流など。物差しとなる参照信号である。
...
**手法は車種非依存である可能性がある。** 「標準PIDをアンカーとし、諸元で幾何を固定し、物理保存則とモード分離で信号を縛る」という枠組みは、FL4 固有の値に依存しない。他の i-MMD 搭載車、他ECU、さらには他メーカーのハイブリッド機構（例：トヨタ THS、日産 e-POWER）や一般の車両の未文書化DIDに対しても、同じ手順を適用できる可能性がある。本レポートの価値は、個別のマッピング結果に加え、この再現可能な解析プロセスを明示的に記録した点にある。
...
本手法は、公開情報と物理法則のみで未文書化の診断データを同定する。特定メーカー・機構に依存しない汎用性が本手法の主要な利点であり、Honda に限らず他社のハイブリッド（トヨタ THS、日産 e-POWER 等）や他ECUの未文書化DID解析へ応用できる可能性がある。今後の課題は、`eng_trq` スケールの独立検証、制動系ECUの探索、およびより高レートなサンプリングによる指令/実測の分離である。
...
- SAE J1979 — E/E Diagnostic Test Modes（標準OBD-II PID）
...
| 1.3 | 2026-08 | 参照PIDの引用規格を SAE J1979-2 → **SAE J1979**（Classic OBD-on-ISO 15765-4）に訂正。本車はサービス01 PIDで応答＝J1979-2(OBDonUDS)には該当しないため。あわせて、特定のメーカー・機構に依存しない汎用性を主利点として整理し、Honda 以外（トヨタ THS・日産 e-POWER 等）への応用可能性を Abstract・§6・§8 に明示。 |
