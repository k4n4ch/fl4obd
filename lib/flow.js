...
function fArrow(svg,pts,kind,active,kw,lx,ly,col,anchor,fs){
  ...
  if(kw!=null){const t=fEl('text',{x:lx,y:ly,'text-anchor':anchor||'middle','font-size':fs||28,'font-weight':'bold','font-family':'Courier New',fill:color,opacity:active?1:0.15},svg);t.textContent=kw.toFixed(1);}
}
...
// MODE(x=312)と同じ列。BAT箱(右端238)とviewBox右端の間の空きに置く。
// 高さは BAT箱の上端に揃える。行の相対位置(+8/+38)は MODE が ENG箱に対して
// 使っているものと同じで、3行が BAT箱の帯(fBY〜fBY+fBH)にちょうど収まる。
const FU_CX=312, FU_Y=fBY;
/*
...
/*
 * 派生量の計算。**レイアウトに依存しない**ので縦長・横長で共有する。
 */
function flowState(){
  ...
  return { mode, raw, modeLabel, isDirect, isSeries, peng, pgen, pbat, pdrive, regen, on,
           genGen, genMot, genMag, dump, trcWheel, busTrc, hasEngWheel, hasGenBranch,
           engGen, engWheel, genBus, auxRaw, auxSat, aux, batBus };
}

/* ── 縦長 380×600（既定）────────────────────────────────── */
function drawFlowPortrait(svg, S){
  const { modeLabel, regen, on, genMot, dump, trcWheel, busTrc, peng, pgen, pdrive,
          engGen, engWheel, genBus, auxSat, aux, batBus } = S;
  ...
  if(D.fuelShow || fuLive){
    ...
    fEl('text',{x:FU_CX,y:FU_Y+8,'text-anchor':'middle','font-size':12,'font-family':'Courier New',fill:'#5a7a90','letter-spacing':2},g).textContent='FUEL';
    fEl('text',{x:FU_CX,y:FU_Y+38,'text-anchor':'middle','font-size':26,'font-weight':'bold','font-family':'Courier New',fill:fc},g).textContent=fuLive?fl.toFixed(1)+' L':'-- L';
    fEl('text',{x:FU_CX,y:FU_Y+56,'text-anchor':'middle','font-size':11,'font-family':'Courier New',fill:'#7a95ad'},g).textContent=(fuLive?D.fuelLvl.toFixed(1):'--')+' %  /'+FUEL_TANK_L+'L';
  }
...
/* ── 横長 640×240 ──────────────────────────────────────────
 * 分割画面など高さの取れない枠向け。車輪を ENG と MOT の間に置くことで機械の接続を
 * 2本とも短い水平にし、電気側は下段の一本の母線に集約している。縦長版と同じ値を出す。
 * 文字の外接矩形・箱・線・枠外の干渉は EV/SERIES/DIRECT の3状態で検査済み。
 */
const gEX=24, gEY=16, gTX=312, gTY=16, gAX=520, gAY=16;     // 上段 ENG / MOT / AUX
const gGX=24, gGY=140, gBX=520, gBY=140;                    // 下段 GEN / BAT
const gWX=216, gWY=44, gJX=360, gJY=168;                    // 車輪 / 合流点
function drawFlowLandscape(svg, S){
  const { modeLabel, regen, on, genMot, dump, trcWheel, busTrc, peng, pgen, pdrive,
          engGen, engWheel, genBus, auxSat, aux, batBus } = S;
  fArrow(svg,[[gEX+fBW,gWY],[gWX-24,gWY]],'mech',on(engWheel),on(engWheel)?engWheel:null,156,30,null,null,20);
  // 力行はモーターが車輪を回す（頭は車輪側）、回生は車輪がモーターを回す（頭は MOT 側）
  if(regen) fArrow(svg,[[gWX+24,gWY],[gTX-4,gWY]],'mech',on(trcWheel),on(trcWheel)?trcWheel:null,274,30,FX_R,null,20);
  else      fArrow(svg,[[gTX-4,gWY],[gWX+24,gWY]],'mech',on(trcWheel),on(trcWheel)?trcWheel:null,274,30,FX_M,null,20);
  if(genMot) fArrow(svg,[[gGX+fBW/2,gGY-6],[gEX+fBW/2,gEY+fBH]],'mech',on(engGen),on(engGen)?engGen:null,82,110,dump?FX_R:FX_M,'start',20);
  else       fArrow(svg,[[gEX+fBW/2,gEY+fBH],[gGX+fBW/2,gGY-6]],'mech',on(engGen),on(engGen)?engGen:null,82,110,null,'start',20);
  if(genMot) fArrow(svg,[[gJX-9,gJY],[gGX+fBW,gJY]],'elec',on(genBus),on(genBus)?genBus:null,235,154,dump?FX_R:null,null,20);
  else       fArrow(svg,[[gGX+fBW,gJY],[gJX-9,gJY]],'elec',on(genBus),on(genBus)?genBus:null,235,154,null,null,20);
  if(regen) fArrow(svg,[[gJX,gTY+fBH],[gJX,gJY-9]],'elec',on(busTrc),on(busTrc)?busTrc:null,350,120,FX_R,'end',20);
  else      fArrow(svg,[[gJX,gJY-9],[gJX,gTY+fBH]],'elec',on(busTrc),on(busTrc)?busTrc:null,350,120,null,'end',20);
  if(batBus>=0) fArrow(svg,[[gJX+9,gJY],[gBX-4,gJY]],'elec',on(batBus),on(batBus)?batBus:null,442,154,regen?FX_R:FX_E,null,20);
  else          fArrow(svg,[[gBX-4,gJY],[gJX+9,gJY]],'elec',on(batBus),on(batBus)?-batBus:null,442,154,null,null,20);
  fArrow(svg,[[gJX+8,gJY-8],[gAX+16,gAY+fBH+20]],'aux',on(aux),on(aux)?aux:null,446,108,null,'end',20);
  fEl('circle',{cx:gJX,cy:gJY,r:8,fill:'#0a0e14',stroke:FX_E,'stroke-width':2.5},svg);
  fEl('circle',{cx:gJX,cy:gJY,r:3,fill:FX_E},svg);
  fNode(svg,gEX,gEY,'ENG',(Math.round(D.engRot)||0)+' rpm',on(peng)||genMot,true);
  fNode(svg,gTX,gTY,'MOT','P_mot',on(pdrive));
  fNode(svg,gAX,gAY,'AUX/AC',on(aux)?(auxSat?'>'+AUX_MAX.toFixed(1)+'kW':aux.toFixed(1)+'kW'):'--',on(aux),true,auxSat?'#ff8844':FX_A);
  fNode(svg,gGX,gGY,'GEN','P_gen',on(pgen));
  fNode(svg,gBX,gBY,'BAT',isNaN(D.soc)?'SOC --':'SOC '+D.soc.toFixed(0)+'%',true,true);
  if(dump) fEl('text',{x:gEX+fBW/2,y:gEY-4,'text-anchor':'middle','font-size':14,'font-weight':'bold','font-family':'Courier New',fill:'#ff5544'},svg).textContent='廃電';
  // 車輪。文字は置かず、その位置に車速を出す
  const wOn=on(trcWheel)||on(engWheel);
  const g=fEl('g',{opacity:wOn?1:0.45},svg);
  fEl('circle',{cx:gWX,cy:gWY,r:20,fill:'none',stroke:'#889','stroke-width':3.5},g);
  fEl('circle',{cx:gWX,cy:gWY,r:5,fill:'#889'},g);
  if(D.vsp!=null&&!isNaN(D.vsp))
    fEl('text',{x:gWX,y:86,'text-anchor':'middle','font-size':24,'font-weight':'bold','font-family':'Courier New',fill:'#9fe8ff'},svg).textContent=Math.round(D.vsp)+' km/h';
  // 下段の文字は箱の左右幅（24〜616）に収める
  const mc={EV:FX_E,SERIES:FX_E,DIRECT:FX_M,TRANS:MODE_COLOR.TRANS}[modeLabel]||'#cfe';
  fEl('text',{x:24,y:220,'text-anchor':'start','font-size':12,'font-family':'Courier New',fill:'#5a7a90','letter-spacing':2},svg).textContent='MODE';
  fEl('text',{x:61,y:220,'text-anchor':'start','font-size':22,'font-weight':'bold','font-family':'Courier New',fill:mc},svg).textContent=modeLabel;
  if(D.fuelShow || (D.fuelLvl!=null && isFinite(D.fuelLvl))){
    const live=(D.fuelLvl!=null && isFinite(D.fuelLvl)), fl=live?D.fuelLvl/100*FUEL_TANK_L:null;
    const fc = !live ? '#7a95ad' : fl<=4 ? '#ff4444' : fl<=8 ? '#ffaa00' : '#dfefff';
    fEl('text',{x:284,y:220,'text-anchor':'start','font-size':12,'font-family':'Courier New',fill:'#5a7a90','letter-spacing':2},svg).textContent='FUEL';
    fEl('text',{x:321,y:220,'text-anchor':'start','font-size':22,'font-weight':'bold','font-family':'Courier New',fill:fc},svg).textContent=live?fl.toFixed(1)+' L':'-- L';
    fEl('text',{x:616,y:220,'text-anchor':'end','font-size':11,'font-family':'Courier New',fill:'#7a95ad'},svg).textContent=(live?D.fuelLvl.toFixed(1):'--')+' % /'+FUEL_TANK_L+'L';
  }
}

/*
 * レイアウトは **svg の viewBox で決まる**。横長（幅>高さ）なら横長版。
 * replay.html / video.html は 380×600 のまま呼ぶので従来どおり縦長になる。
 */
function drawFlow(svg){
  ...
  fDefs(svg);
  const S=flowState();
  const vb=((svg.getAttribute&&svg.getAttribute('viewBox'))||'0 0 380 600').split(/[ ,]+/).map(Number);
  if(vb[2]>vb[3]) drawFlowLandscape(svg,S); else drawFlowPortrait(svg,S);
}
