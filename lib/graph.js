 ...
 *   panel  = { h, zero, series:[...], yl:[min,max]|null, yr:[min,max]|null, fmtL, fmtR,
 *                minL, minR (自動スケールの最小スパン), zeroL, zeroR (0 を必ず含める) }
 *   series = { name, color, axis:'L'|'R', pts:[[t,v],...], w, fill, fmt, segColor }
 ...
 */
function autoRange(series, axis, t0, t1, minSpan = 0, zeroBase = false) {
  ...
  if (minSpan > 0 && hi - lo < minSpan) {        // 狭すぎたら中心を保って広げる
    const c = zeroBase && lo >= 0 ? minSpan / 2 : (lo + hi) / 2;
    lo = c - minSpan / 2; hi = c + minSpan / 2;
    if (zeroBase) { lo = Math.min(lo, 0); hi = lo + minSpan; }
  }
  return [lo, hi];
}
...
export function drawGraph(svg, panels, tNow, span, W, H) {
  ...
  for (const p of panels) {
    ...
    // 枠と目盛り
    gEl('rect', { x: PAD_L, y: yt, width: plotW, height: ph, fill: '#0c1a24', stroke: '#16303f', 'stroke-width': 1 }, svg);
    const rngL = p.yl || autoRange(p.series, 'L', t0, t1, p.minL, p.zeroL);
    const rngR = p.yr || autoRange(p.series, 'R', t0, t1, p.minR, p.zeroR);
    ...
    for (const s of p.series) {
      ...
      const [a, b] = sliceRange(s.pts, t0, t1);
      if (b <= a) continue;
      // 区間ごとに色が変わる系列（車速をモード色で塗る等）。マップのルート表現と同じ語彙にする。
      if (s.segColor) {
        const runs = [];
        let cur = null;
        for (let i = a; i <= b; i++) {
          ...
          const col = s.segColor(i);
          const pt = [PX(s.pts[i][0]), Math.max(yt, Math.min(yb, mapY(v, r)))];
          if (!cur || cur.col !== col) {
            if (cur) cur.pts.push(pt);                    // 色境界で線を切らない（1点重ねる）
            cur = { col, pts: [pt] }; runs.push(cur);
          } else cur.pts.push(pt);
        }
        for (const rn of runs) {
          if (rn.pts.length < 2) continue;
          gEl('path', { d: 'M' + rn.pts.map((q) => q[0].toFixed(1) + ' ' + q[1].toFixed(1)).join(' L'),
      ...
      for (let i = a; i <= b; i++) {
        const v = s.pts[i][1];
        if (!Number.isFinite(v)) { pen = false; continue; }   // 欠測でパスを切る
        const cx = PX(s.pts[i][0]), cy = Math.max(yt, Math.min(yb, mapY(v, r)));
      ...
      if (s.fill) {
        ...
        gEl('path', { d: d + `L${PX(s.pts[b][0]).toFixed(1)} ${base} L${PX(s.pts[a][0]).toFixed(1)} ${base} Z`,
                      fill: s.color, opacity: 0.18, stroke: 'none' }, svg);
      }
      gEl('path', { d, fill: 'none', stroke: s.color, 'stroke-width': s.w || 1.6,
                    'stroke-linejoin': 'round', 'stroke-linecap': 'round' }, svg);
    }
    // 軸ラベル（上下端の値）と系列名
    const lab = (x, yy, txt, col, anchor) =>
      gEl('text', { x, y: yy, 'text-anchor': anchor, 'font-size': 10, 'font-family': 'Courier New', fill: col }, svg).textContent = txt;
    ...
    for (const s of p.series) {
      const r = (s.axis === 'R') ? rngR : rngL;
      let cur = NaN;
      if (s.pts.length) { const [, b] = sliceRange(s.pts, t0, tNow); if (b >= 0 && s.pts[b]) cur = s.pts[b][1]; }
      const txt = s.name + (Number.isFinite(cur) ? ' ' + (s.fmt ? s.fmt(cur) : cur.toFixed(1)) : '');
      const t = gEl('text', { x: lx, y: yt + 11, 'font-size': 11, 'font-family': 'Courier New',
  ...
  // 時間軸の目安
  gEl('text', { x: PAD_L + 2, y: H - 3, 'font-size': 10, 'font-family': 'Courier New', fill: '#4a6a80' }, svg)
    .textContent = '-' + (span / 2).toFixed(0) + 's';
  gEl('text', { x: PAD_L + plotW - 2, y: H - 3, 'text-anchor': 'end', 'font-size': 10, 'font-family': 'Courier New', fill: '#4a6a80' }, svg)
    .textContent = '+' + (span / 2).toFixed(0) + 's';
}
